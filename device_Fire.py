...
class TFire():
    ...
    def OnInit(self):
        ...
        if self.FLFireDeviceCount == 1: # first (or only) FL Fire unit. Load the settings
            self.LoadKeyColors()
            self.FLFirePadBrightness = 128
            self.FLFirePadSaturation = 128
        ...
        if not event.handled:
                ...
                if event.data1 == IDJogWheel:
                    ...
                    if self.CurrentMode == ModeStepEdit:
                        if self.step_edit_handler.EditMode:
                            txt = self.step_edit_handler.OnKnobEdit(event.data1, self.AdaptKnobVal(event.outEv), self.ShiftHeld)
                            if txt:
                                self.DisplayTimedText(txt)
                        ...
                        if event.data1 == IDKnob1:
                            delta = self.AdaptKnobVal(event.outEv)
                            vol_id = REC_Mixer_Vol + mixer.getTrackPluginId(self.CurrentMixerTrack, 0)
                            cur_vol = mixer.getTrackVolume(self.CurrentMixerTrack)
                            
                            if delta > 0 and cur_vol >= 0.8:
                                pass # Plafond à 100% (0.8 dans le code FL) atteint
                            else:
                                HandleKnob(vol_id, delta, mixer.getTrackName(self.CurrentMixerTrack) + ' Vol', False)
                                # Si le cran du potard a fait dépasser les 100%, on clamp la valeur à 0.8
                                if mixer.getTrackVolume(self.CurrentMixerTrack) > 0.8:
                                    mixer.setTrackVolume(self.CurrentMixerTrack, 0.8)
                                    self.DisplayBar(mixer.getTrackName(self.CurrentMixerTrack) + ' Vol', 0.8, False)
                        ...
                        if event.data1 == IDKnob3:
                            HandleKnob(REC_Mixer_SS + mixer.getTrackPluginId(self.CurrentMixerTrack, 0), self.AdaptKnobVal(event.outEv), mixer.getTrackName(self.CurrentMixerTrack) + ' Stereo', True)
            ...
            if event.midiId in [MIDI_NOTEON, MIDI_NOTEOFF]:
                ...
                # navigation
                    ...
                    elif (event.midiId == MIDI_NOTEON) & (self.CurrentMode == ModeDrum):
                        txt = self.fl_control_handler.OnPatternUp()
                        if txt:
                            self.DisplayTimedText(txt)
                    elif (event.midiId == MIDI_NOTEON) & (patterns.patternNumber() > 1) & (self.CurrentMode == ModeStepSeq):
                    ...
                    if (event.midiId == MIDI_NOTEON) & (self.CurrentMode == ModeStepEdit):
                        ...
                        txt = self.fl_control_handler.OnPatternDown()
                        if txt:
                            self.DisplayTimedText(txt)
                    elif (event.midiId == MIDI_NOTEON) & (patterns.patternNumber() < patterns.patternMax()) & (self.CurrentMode == ModeStepSeq):
                        if not self.AltHeld:
                            patterns.jumpToPattern(patterns.patternNumber() + 1)
                        else:
                    ...
                    # We want Grid L/R to always be red (DualColorFull1), but we can make them 
                    # momentarily brighter when pressed. Wait, if we want them static red:
                    # Let's just restore them to DualColorFull1
                    if event.midiId == MIDI_NOTEON:
                        # Full brightness when pressed
                        self.SendCC(event.data1, SingleColorFull)
                    else:
                        # Return to normal red state when released
                        self.SendCC(event.data1, DualColorFull1)
                ...
                # button modes
                    ...
                    if self.ShiftHeld and self.CurrentMode != ModeStepEdit:
    ...
    def ClearAllButtons(self):
        ...
        # grid (always red)
        self.SendCC(IDBankL, DualColorFull1)
        self.SendCC(IDBankR, DualColorFull1)
        # patterns
    ...
    def UpdateCurrentModeBtns(self):
        ...
        if self.CurrentMode == ModeStepEdit:
            ...
            self.SendCC(IDPatternDown, SingleColorHalfBright)
