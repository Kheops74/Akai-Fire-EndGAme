...
def _NormalizeFixedSendKeys(entries):
    ...
    names = (
        'row3_10', 'row3_11', 'row3_12', 'row3_13',
        'row4_10', 'row4_11', 'row4_12', 'row4_13',
    )
    result = {}
    for name in names:
...
class FLControlMode(FireModeBase):
    ...
    def __init__(self, fire_device):
        ...
        self._patternPage = 0         # offset des patterns (0 = 1-16, 1 = 17-32, etc.)
        self.NativeColorMode = False  # Toggle pour les couleurs FL natives

    def OnPatternUp(self):
        """Handle Pattern Up button to shift pattern page by 16."""
        if self._patternPage > 0:
            self._patternPage -= 1
            self.Refresh()
            return f"Patterns {(self._patternPage * 16) + 1}-{(self._patternPage * 16) + 16}"
        return ""

    def OnPatternDown(self):
        """Handle Pattern Down button to shift pattern page by 16."""
        max_pat = patterns.patternMax()
        if (self._patternPage + 1) * 16 < max_pat:
            self._patternPage += 1
            self.Refresh()
            return f"Patterns {(self._patternPage * 16) + 1}-{(self._patternPage * 16) + 16}"
        return ""
    ...
    def _GetFixedSendKey(self, idx):
        """Return configured fixed SendKey entry for row 3 or row 4."""
        row = idx // 10
        pad_num = 10 + (idx % 10)
        return FIXED_SENDKEYS.get('row' + str(row + 1) + '_' + str(pad_num), ('', False, False, False, '', COL_OFF))
    ...
    def _GetPadInfo(self, pad_num):
        ...
        if row == 0:
            ...
            # Row 2: PL tools (0-8 swappable), SendKey fixes (9-12), API nav (13-15)
            if col < SWAPPABLE_PAD_COUNT:
                return (FLC_PL_TOOL, col)
            elif col <= 12:
                return (FLC_FIXED_SENDKEY, 20 + (col - 9))
            elif col == 13:
            ...
            # Row 3: PR tools (0-8 swappable), SendKey fixes (9-12), API nav (13-15)
            if col < SWAPPABLE_PAD_COUNT:
                return (FLC_PR_TOOL, col)
            elif col <= 12:
                return (FLC_FIXED_SENDKEY, 30 + (col - 9))
            elif col == 13:
    ...
    def _HandlePatternSelect(self, col):
        """Select pattern based on page (col 0-15 -> pat 1-16 or 17-32 etc.)."""
        pat = (self._patternPage * 16) + col + 1
    ...
    def _GetPadColor(self, pad_num, blinkOn=False):
        ...
        if func == FLC_MIXER_MUTE:
            ...
            state_color = muted_color if mixer.isTrackMuted(track) else base_color
            
            if track == mixer.trackNumber():
                return state_color if not blinkOn else COL_MIXER_SEL
            return state_color
            ...
            state_color = muted_color if channels.isChannelMuted(ch) else base_color
            
            if ch == channels.channelNumber():
                return state_color if not blinkOn else COL_CHAN_SEL
            return state_color

        elif func == FLC_PATTERN_SEL:
            pat = (self._patternPage * 16) + idx + 1
            curPat = patterns.patternNumber()
            
            if pat > patterns.patternCount():
                if pat == curPat:
                    return COL_PAT_EMPTY if not blinkOn else COL_PAT_ACTIVE
            ...
            state_color = self._DimColor(self._BoostColor(native_color), 0.5) if use_native else COL_PAT_EXIST
            active_color = self._BoostColor(native_color) if use_native else COL_PAT_ACTIVE
            
            if pat == curPat:
                return state_color if not blinkOn else active_color
            return state_color
    ...
    def Refresh(self):
        ...
        dataOut = bytearray(0)
        blinkOn = self.fire.BlinkTimer < (BlinkSpeed)

        for row in range(4):
            for col in range(16):
                pad_num = row * PadsStride + col
                color = self._GetPadColor(pad_num, blinkOn)
