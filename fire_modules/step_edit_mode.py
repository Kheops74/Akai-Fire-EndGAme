...
class StepEditMode(FireModeBase):
    ...
    def OnKnobEdit(self, knobId, increment, shift_held=False):
        ...
        if knobId == IDKnob1:
            ...
            return self._EditDelay(increment, shift_held)
        return ''
    ...
    def _EditDelay(self, increment, shift_held):
        """Change time delay (offset) of all selected notes.
        Without shift: adjust forward delay (0 to 127).
        With shift: adjust backward delay (0 to -128)."""
        for step, pitch in self.SelectedPads:
            note = self.GetNote(step, pitch)
            if note is not None:
                if shift_held:
                    # Direction négative (en arrière)
                    if note.delay > 0:
                        note.delay = 0 # On repasse par 0 si on était en positif
                    note.delay = max(-128, min(0, note.delay - increment * 4))
                else:
                    # Direction positive (en avant)
                    if note.delay < 0:
                        note.delay = 0 # On repasse par 0 si on était en négatif
    ...
    def _DelayToTickOffset(self, delay, ticksPerStep):
        """Convert internal delay (-128 to 127) to signed tick offset.
        delay=0 -> 0, delay=127 -> +ticksPerStep, delay=-128 -> -ticksPerStep"""
        if delay == 0:
            return 0
        return int(delay * ticksPerStep / 128)
    ...
    def OnIdle(self):
        ...
        # --- Handle Channel or Pattern Changes for Autosave ---
        curChan = channels.channelNumber()
        curPat = patterns.patternNumber()
        
        if not hasattr(self, '_lastWatchedChan'):
            self._lastWatchedChan = curChan
            self._lastWatchedPat = curPat
            
        if curChan != self._lastWatchedChan or curPat != self._lastWatchedPat:
            # L'utilisateur a changé de pattern ou de channel pendant qu'il était en mode StepEdit
            # On sauvegarde d'abord l'état de l'ANCIEN channel/pattern
            try:
                if hasattr(self, 'SaveJsonFile') and hasattr(self, 'LoadJsonFile'):
                    saveNameOld = f'autosave_chan{self._lastWatchedChan}_pat{self._lastWatchedPat}'
                    self.SaveJsonFile(saveNameOld)
                    
                    # Ensuite on charge le NOUVEAU
                    self._lastWatchedChan = curChan
                    self._lastWatchedPat = curPat
                    ...
                    saveNameNew = f'autosave_chan{curChan}_pat{curPat}'
                    loaded = False
                    saveDir = self._GetSavePath()
                    if saveDir and os.path.exists(os.path.join(saveDir, saveNameNew + '.json')):
                        loaded = self.LoadJsonFile(saveNameNew)
                        
                    if not loaded and not self.ChordMode:
                        self.SyncFromChannelRackMono(False)
                        
                    self.Refresh()
            except Exception as e:
                print('Autosave sync error: ' + str(e))
    ...
    def OnActivate(self):
        ...
        # Tente de charger un autosave pour ce pattern/channel
        loaded = False
        try:
            saveName = f'autosave_chan{curChan}_pat{curPat}'
            # Check if file exists to avoid unnecessary error prints
            saveDir = self._GetSavePath()
            if saveDir and os.path.exists(os.path.join(saveDir, saveName + '.json')):
                loaded = self.LoadJsonFile(saveName)
        except Exception:
            pass

        if not loaded and not self.ChordMode:
            self.SyncFromChannelRackMono(False) # changed from True to False to preserve dirty grid
        self.Refresh()
        self._UpdateFLView()
        self._DispatchStepEditSync()
    ...
    def OnDeactivate(self):
        ...
        # Sauvegarde automatique de l'état pour ce pattern/channel
        try:
            chan = channels.channelNumber()
            pat = patterns.patternNumber()
            saveName = f'autosave_chan{chan}_pat{pat}'
            self.SaveJsonFile(saveName)
        except Exception:
            pass
